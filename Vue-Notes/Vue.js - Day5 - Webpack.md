...
(类似于地球上的珠穆朗玛峰，只有在它周围才能看见它，所以使用Gulp建立task任务，只能处理于周边，如果还想处理其他的就新建一个task任务然后再把这些任务关联起来，虽然Gulp小巧、方便、灵活，但是如果是一个大型项目的话就要用Gulp建很多个任务关联起来很麻烦，所以不适用于用这个)
2. 使用Webpack， 是基于整个项目进行构建的；
（相当于卫星，直接扫视地球，从一个更宏观的角度来对待项目）
